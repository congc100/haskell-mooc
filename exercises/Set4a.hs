...
longest ls = go 0 [] ls
    where
        go len res [] = res
        go len res (l:ls)
            | length l > len  = go (length l) l ls
            | length l == len = go len (min res l) ls
            | otherwise       = go len res ls
...
incrementKey key []            = []
incrementKey key ((k, v) : ps) =
    if k == key
        then (k, v+1) : incrementKey key ps
        else (k, v) : incrementKey key ps
...
freqs xs = foldr helper (Map.fromList []) xs
    where
        helper key res = Map.alter inc key res
        inc Nothing    = Just 1
...
transfer from to amount bank
    ...
    | otherwise                                = r1
        where
            r0 = Map.adjustWithKey (\k v -> v - amount) from bank
            r1 = Map.adjustWithKey (\k v -> v + amount) to r0
...
maxIndex arr = go (head ids) (arr ! head ids) 1 arr
    where
        ids = Data.Array.indices arr
        go index value cnt arr
            | cnt == length ids = index
            | otherwise         =
                if (arr ! (ids !! cnt)) > value
                    then go (ids !! cnt) (arr ! (ids !! cnt)) (cnt + 1) arr
                    else go index value (cnt + 1) arr

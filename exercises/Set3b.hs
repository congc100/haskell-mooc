...
sums i = go i 1 1
    where
        go end cur s
            | cur == end = [s]
            | otherwise  = s : go end (cur+1) (s+cur+1)
...
maybeMap f []     = []
maybeMap f (x:xs) = go (f x) xs
    where
        go Nothing xs  = maybeMap f xs
        go (Just v) xs = v : maybeMap f xs

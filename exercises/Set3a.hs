...
firstHalf s = take n s
    where len = length s
          n = if even len then div len 2 else div len 2 + 1
...
joinToLength n xs = [first ++ last | first <- xs, last <- xs,
    length first + length last == n]
...
sumRights xs = sum $ map (either (\x -> 0) (\x -> x)) xs
...
interpreter commands = go 0 0 [] commands
    where go x y r []     = r
          go x y r (c:cs) = case c of
            "up"     -> go x (y+1) r cs
            "down"   -> go x (y-1) r cs
            "left"   -> go (x-1) y r cs
            "right"  -> go (x+1) y r cs
            "printX" -> go x y (r ++ [show x]) cs
            "printY" -> go x y (r ++ [show y]) cs

...
totalPrice (MkShoppingEntry n p c) = p * fromIntegral c
...
data Person = DefPerson Int String
  deriving Show

-- fred is a person whose name is Fred and age is 90
fred :: Person
fred = DefPerson 90 "Fred"

-- getName returns the name of the person
getName :: Person -> String
getName (DefPerson age name) = name

-- getAge returns the age of the person
getAge :: Person -> Int
getAge (DefPerson age name) = age

-- setName takes a person and returns a new person with the name changed
setName :: String -> Person -> Person
setName name (DefPerson age _) = DefPerson age name

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age (DefPerson _ name) = DefPerson age name
...
tick :: UpDown -> UpDown
tick (UpDownInc n) = UpDownInc (n + 1)
tick (UpDownDec n) = UpDownDec (n - 1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle (UpDownInc n) = UpDownDec n
toggle (UpDownDec n) = UpDownInc n
...
rgb Green = [0, 1, 0]
rgb Blue  = [0, 0, 1]
rgb (Mix a b) = go (rgb a) (rgb b)
  where
    go [a1, a2, a3] [b1, b2, b3] = [(a1+b1)/2, (a2+b2)/2, (a3+b3)/2]
rgb (Invert c) = go (rgb c)
  where
    go [c1, c2, c3] = [1-c1, 1-c2, 1-c3]
